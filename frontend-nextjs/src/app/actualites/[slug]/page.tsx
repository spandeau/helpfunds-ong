      ...
      <div className="relative h-[55vh] min-h-[400px] overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
          <div className="max-w-4xl mx-auto">
            ...
            <h1 className="font-heading font-bold text-white text-3xl md:text-4xl mb-4 leading-tight">{post.title}</h1>
            <div className="flex flex-wrap items-center gap-4 text-white/60 text-sm">
              <div className="flex items-center gap-2">
                <div className="w-7 h-7 bg-white/20 rounded-full flex items-center justify-center text-white text-xs font-bold">{post.authorAvatar}</div>
                <span className="font-medium text-white/80">{post.author}</span>
              </div>
              <span>·</span>
              <span className="flex items-center gap-1"><Calendar className="w-4 h-4" />{new Date(post.publishedAt).toLocaleDateString("fr-FR", { day: "numeric", month: "long", year: "numeric" })}</span>
              {post.type === "article" && (<><span>·</span><span className="flex items-center gap-1"><Clock className="w-4 h-4" />{readTime} min de lecture</span></>)}
            </div>
          </div>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-secondary-50 border border-secondary-100 rounded-2xl p-5 flex flex-col sm:flex-row gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="font-bold text-neutral-900">{new Date(post.eventDate).toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}</p>
                    </div>
                  </div>
                  {post.eventLocation && (
                    <div className="flex items-center gap-3">
              ...
              {post.images && post.images.length > 0 && (
                <PhotoGallery images={post.images} title={post.title} type={post.type} />
              )}
            ...
            <div className="lg:col-span-1 space-y-5">
              <div className="bg-white rounded-2xl p-5 border border-neutral-100 shadow-sm">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-primary-600 rounded-full flex items-center justify-center text-white font-bold">{post.authorAvatar}</div>
                  <div>
                ...
                <div className="bg-white rounded-2xl p-5 border border-neutral-100 shadow-sm">
                  ...
                  <div className="space-y-4">
                    {relatedPosts.map((related) => (
                      <Link key={related.id} href={`/actualites/${related.slug}`} className="flex gap-3 group">
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs text-neutral-400 mb-1">{new Date(related.publishedAt).toLocaleDateString("fr-FR", { day: "numeric", month: "short" })}</p>
                          <h4 className="text-sm font-bold text-neutral-900 group-hover:text-primary-600 transition-colors line-clamp-2 leading-tight">{related.title}</h4>
                        </div>
                      </Link>
                    ))}
                  </div>
