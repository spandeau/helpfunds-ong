...
export default function ActualitesPage() {
  ...
  const filtered = useMemo(() => {
    return BLOG_POSTS.filter((p) => {
      const matchSearch = p.title.toLowerCase().includes(search.toLowerCase()) || p.excerpt.toLowerCase().includes(search.toLowerCase()) || (p.country?.toLowerCase().includes(search.toLowerCase()) ?? false);
      ...
      <section className="bg-gradient-to-br from-primary-950 via-primary-900 to-primary-800 pt-32 pb-16">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-xl text-white/70 max-w-2xl mx-auto">
            Suivez nos actions sur le terrain, nos evenements et decouvrez l impact concret de vos dons.
          </p>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex flex-col md:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input type="text" placeholder="Rechercher un article, un pays..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2.5 rounded-xl border-2 border-neutral-200 focus:outline-none focus:border-primary-500 text-sm" />
            ...
            <div className="flex flex-wrap gap-2">
              ...
              {hasFilters && (
                <button onClick={resetFilters} className="px-3 py-2.5 rounded-xl border-2 border-red-200 text-red-600 text-sm font-medium hover:bg-red-50 transition-colors">
                  <X className="w-4 h-4" />
                </button>
              )}
            </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2">
                      <div className="relative h-64 md:h-full min-h-[300px]">
                        ...
                        <div className="absolute top-4 left-4">
                          {(() => { const config = TYPE_CONFIG[featured[0].type]; const Icon = config.icon; return (<span className={`inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full ${config.color}`}><Icon className="w-3.5 h-3.5" />{config.label}</span>); })()}
                        </div>
                      </div>
