...
import { Heart, User, Mail, MessageSquare, CreditCard, Repeat, Gift, Check, ChevronLeft, ChevronRight, Shield, ArrowRight } from "lucide-react";
...
const IMPACT_SLIDES = [
  { id: 1, title: "Aide alimentaire", description: "Distribution de repas chauds aux familles vulnerables dans les zones de crise.", image: "https://images.unsplash.com/photo-1488521787991-ed7bbaae773c?w=800&q=80", stat: "12 000 repas / mois", badge: "Alimentation", href: "/projets" },
  { id: 2, title: "Acces aux soins", description: "Consultations medicales gratuites pour les communautes isolees.", image: "https://images.unsplash.com/photo-1576091160399-112ba8d25d1d?w=800&q=80", stat: "1 200 patients / mois", badge: "Sante", href: "/projets" },
  { id: 3, title: "Education des enfants", description: "Fournitures scolaires pour les enfants defavorises.", image: "https://images.unsplash.com/photo-1497486751825-1233686d5d80?w=800&q=80", stat: "3 500 enfants", badge: "Education", href: "/projets" },
  { id: 4, title: "Projets communautaires", description: "Construction de puits et projets agricoles durables.", image: "https://images.unsplash.com/photo-1541544537156-7627a7a4aa1c?w=800&q=80", stat: "45 villages", badge: "Communaute", href: "/projets" },
  { id: 5, title: "Aide aux familles", description: "Soutien psychologique pour les familles en detresse.", image: "https://images.unsplash.com/photo-1469571486292-0ba58a3f068b?w=800&q=80", stat: "800 familles", badge: "Famille", href: "/projets" },
  { id: 6, title: "Eau potable", description: "Installation de points d eau dans les villages isoles.", image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=800&q=80", stat: "2 500 personnes", badge: "Eau", href: "/projets" },
];
    ...
    <div className="mt-8">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <p className="text-neutral-400 text-xs mt-0.5">Cliquez pour decouvrir chaque action sur le terrain</p>
        </div>
      ...
      <div ref={emblaRef} className="overflow-hidden">
        <div className="flex gap-3">
          {IMPACT_SLIDES.map((slide, index) => (
            <Link key={slide.id} href={slide.href} className="flex-none w-[calc(33.333%-8px)] min-w-[160px] group">
              <div className={`rounded-2xl overflow-hidden border-2 transition-all duration-300 h-full ${selectedIndex === index ? "border-primary-500 shadow-lg" : "border-neutral-100 hover:border-primary-300 shadow-sm"}`}>
                ...
                <div className="bg-white p-3">
                  ...
                  <div className="flex items-center gap-1 text-primary-600 text-[10px] font-bold">
  ...
  return (
    <div className="my-5 p-4 bg-neutral-50 rounded-xl border border-neutral-100">
      ...
      <div className="flex flex-wrap gap-2 items-center">
        ...
        <div className="h-8 px-3 bg-[#003087] rounded-lg flex items-center shadow-sm">
            ...
            {step === 1 && (
              <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-6 md:p-8">
                ...
                <div className="relative mb-4">
                  <input type="number" placeholder="Montant en euros" value={customAmount} onChange={(e) => { setCustomAmount(e.target.value); setSelectedAmount(0); }} className="w-full px-4 py-3 pr-12 rounded-xl border-2 border-neutral-200 focus:outline-none focus:border-primary-500 text-lg font-semibold" min="1" />
                ...
                <button onClick={() => finalAmount > 0 && setStep(2)} disabled={finalAmount <= 0} className="w-full bg-primary-600 hover:bg-primary-700 disabled:bg-neutral-200 disabled:cursor-not-allowed text-white font-bold py-4 rounded-2xl text-lg transition-all hover:shadow-lg mb-2">
                ...
                <div className="flex flex-col gap-5">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                      ...
                      <div className="relative"><User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" /><input type="text" placeholder="Jean" value={formData.firstName} onChange={(e) => setFormData({...formData, firstName: e.target.value})} className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-neutral-200 focus:outline-none focus:border-primary-500" /></div>
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-neutral-700 mb-1.5">Nom</label>
                      <div className="relative"><User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" /><input type="text" placeholder="Dupont" value={formData.lastName} onChange={(e) => setFormData({...formData, lastName: e.target.value})} className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-neutral-200 focus:outline-none focus:border-primary-500" /></div>
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-neutral-700 mb-1.5">Email</label>
                    <div className="relative"><Mail className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-neutral-400" /><input type="email" placeholder="jean.dupont@email.com" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-neutral-200 focus:outline-none focus:border-primary-500" /></div>
                  ...
                  <div>
                    ...
                    <div className="relative"><MessageSquare className="absolute left-3 top-3 w-4 h-4 text-neutral-400" /><textarea placeholder="Laissez un message..." value={formData.message} onChange={(e) => setFormData({...formData, message: e.target.value})} rows={3} className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-neutral-200 focus:outline-none focus:border-primary-500 resize-none" /></div>
                  </div>
                  <button onClick={() => setAnonymous(!anonymous)} className="flex items-center gap-3 p-4 rounded-xl border-2 border-neutral-200 hover:border-neutral-300 transition-all text-left">
                    <div className={`w-5 h-5 rounded border-2 flex items-center justify-center flex-shrink-0 ${anonymous ? "border-primary-600 bg-primary-600" : "border-neutral-300"}`}>{anonymous && <Check className="w-3 h-3 text-white" />}</div>
                    <div><p className="text-sm font-semibold text-neutral-700">Don anonyme</p><p className="text-xs text-neutral-400">Votre nom ne sera pas affiche publiquement</p></div>
                  </button>
                </div>
            ...
            {step === 3 && (
              <div className="bg-white rounded-2xl border border-neutral-100 shadow-sm p-6 md:p-8">
                ...
                <div className="bg-neutral-50 rounded-xl p-5 mb-6 space-y-3">
                  <div className="flex justify-between"><span className="text-neutral-500 text-sm">Type</span><span className="font-semibold">{donationType === "mensuel" ? "Don mensuel" : "Don unique"}</span></div>
                  ...
                  <div className="border-t pt-3 flex justify-between"><span className="text-neutral-500 text-sm">Donateur</span><span className="font-semibold">{anonymous ? "Anonyme" : formData.firstName + " " + formData.lastName}</span></div>
                  <div className="flex justify-between"><span className="text-neutral-500 text-sm">Email</span><span className="font-semibold text-sm">{formData.email}</span></div>
                </div>
                <div className="bg-primary-50 border border-primary-100 rounded-xl p-5 mb-4">
                  ...
                  <div className="space-y-3">
                    <input type="text" placeholder="Numero de carte" className="w-full px-4 py-3 rounded-xl border-2 border-primary-100 bg-white focus:outline-none text-sm" />
                    <div className="grid grid-cols-2 gap-3">
                      <input type="text" placeholder="MM/AA" className="w-full px-4 py-3 rounded-xl border-2 border-primary-100 bg-white focus:outline-none text-sm" />
                      <input type="text" placeholder="CVV" className="w-full px-4 py-3 rounded-xl border-2 border-primary-100 bg-white focus:outline-none text-sm" />
                    </div>
                  </div>
                </div>
                <PaymentLogos />
                <div className="flex gap-4">
                  ...
                  <button onClick={() => setSubmitted(true)} className="flex-1 bg-secondary-600 hover:bg-secondary-700 text-white font-bold py-4 rounded-2xl flex items-center justify-center gap-2">
          ...
          <div className="lg:col-span-1">
            <div className="bg-gradient-to-br from-primary-900 to-primary-950 rounded-2xl p-6 text-white sticky top-28">
              ...
              <div className="border-t border-white/20 pt-4 mb-4">
                <div className="flex justify-between mb-2"><span className="text-sm text-white/70">Votre don</span><span className="font-bold text-secondary-400 text-lg">{finalAmount > 0 ? finalAmount + "€" : "-"}{donationType === "mensuel" && finalAmount > 0 ? "/mois" : ""}</span></div>
