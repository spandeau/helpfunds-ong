...
import { PROJECTS, COUNTRIES, CATEGORIES } from "@/lib/projects-data";
...
const CAT_ICONS: Record<string, string> = {
  sante: "🏥", education: "📚", eau: "💧",
  alimentation: "🌾", logement: "🏠", economie: "💼",
};
    ...
    <div className="bg-neutral-50 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div className="flex flex-wrap gap-3 mb-6">
              {Object.entries(projectsByCountry).map(([country, projs]) => (
                ...
                >
                  ...
                  {country}
                  <span className={`px-1.5 py-0.5 rounded-full text-xs font-bold ${activeCountry === country ? "bg-white/20" : "bg-neutral-100"}`}>
                    {projs.length}
                  </span>
                </button>
              ))}
                      ...
                      <Link key={p.id} href={`/projets/${p.slug}`}
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <span className="text-xs font-semibold text-neutral-500">{CAT_ICONS[p.category]} {CAT_LABELS[p.category]}</span>
                          </div>
                          ...
                          <div className="mt-1 h-1.5 bg-neutral-100 rounded-full overflow-hidden">
                            <div className="h-full bg-gradient-to-r from-primary-500 to-secondary-500 rounded-full"
                              style={{ width: `${getProgress(p.raisedAmount, p.goalAmount)}%` }} />
                          </div>
                        </div>
                        <ArrowRight className="w-4 h-4 text-neutral-300 group-hover:text-primary-600 transition-colors flex-shrink-0" />
                      </Link>
                    ))}
                  </div>
                </motion.div>
        ...
        {!hasFilters && (
          <div className="mb-14">
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-heading font-bold text-neutral-900">Explorer par domaine</h2>
            </div>
              ...
              {CATEGORIES.map((cat) => {
                ...
                return (
                  <button key={cat} onClick={() => setSelectedCategory(cat)}
                    className="bg-white rounded-2xl p-4 border border-neutral-100 hover:border-primary-200 hover:shadow-md transition-all text-center">
                    <div className="text-2xl mb-2">{CAT_ICONS[cat]}</div>
                    <div className="font-semibold text-neutral-900 text-xs leading-tight">{CAT_LABELS[cat]}</div>
                    <div className="text-xs text-neutral-400 mt-1">{count} projet{count > 1 ? "s" : ""}</div>
                  </button>
                );
              })}
            </div>
          ...
          <div>
            <div className="flex items-center gap-3 mb-6">
              ...
              <h2 className="text-xl font-heading font-bold text-neutral-900">
                {hasFilters ? "Resultats" : "Autres projets"}
              </h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        <div className="mt-16 bg-gradient-to-r from-primary-900 to-primary-950 rounded-2xl p-8 md:p-12 text-center">
          ...
          <Link href="/don"
            className="inline-flex items-center gap-2 bg-secondary-600 hover:bg-secondary-500 text-white font-bold px-10 py-4 rounded-2xl transition-all hover:-translate-y-1 shadow-lg text-lg">
    ...
    >
      <div className="relative h-48 overflow-hidden">
        ...
        <div className="absolute bottom-3 left-3 flex items-center gap-1.5">
          ...
          <span className="text-white text-xs font-medium">{project.region}, {project.country}</span>
        </div>
      </div>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mb-4">
          ...
          <div className="h-2.5 bg-neutral-100 rounded-full overflow-hidden">
            <motion.div
              ...
              className={`h-full rounded-full ${
                progress === 100 ? "bg-secondary-500" :
                project.status === "urgent" ? "bg-gradient-to-r from-red-500 to-orange-500" :
          ...
          <div className="flex justify-between mt-1.5 text-xs text-neutral-400">
            ...
            {project.status !== "termine" && (
              <span className="text-orange-600 font-medium">{formatCurrency(project.goalAmount - project.raisedAmount)} manquants</span>
            )}
          </div>
        </div>
        ...
        <div className="flex gap-2">
          ...
          {project.status !== "termine" && (
            <Link href="/don"
              className="flex items-center justify-center gap-1 bg-secondary-50 hover:bg-secondary-100 text-secondary-700 font-semibold py-2.5 px-3 rounded-xl text-sm border border-secondary-200 transition-all">
